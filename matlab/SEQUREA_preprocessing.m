...
function [] = SEQUREA_preprocessing()
...
%   
%   for itemp = 1:6
%   copyfile(strcat('F:\CBASP_reward\Individuals\K_BN01_t2\T1\Template_',num2str(itemp),'.nii'),strcat('F:\CBASP_reward\Templates\Template_',num2str(itemp),'_Reward_26Kvs26P.nii'));
%   end 
%   
%   dartel_existing()
% 

for isub = 1:1%length(subjs)
 

   direc_funct = fullfile(study_dir,subjs(isub).name,strcat('Functional_',subjs(isub).name));
   % spm_select('clearvfiles');
   

   direc_struct = fullfile(study_dir,subjs(isub).name,strcat('Structural_',subjs(isub).name)); %%%% ADJUST
...
function [] = slicetiming()
    ...
    jobs{1}.temporal{1}.st.prefix = 'a_';
    
    %spm_select('clearvfiles');
    
     [raw_func_filenames_ra, dirs] = spm_select('List', direc_funct, '^2013.*\.nii$');
    filenames_for_st = strcat(direc_funct,filesep,raw_func_filenames_ra); 
    
    jobs{1}.temporal{1}.st.scans{1} =  cellstr(filenames_for_st) ;
    ...
    disp(strcat({'@ Slice-Timing for '},subjs(isub).name, {' done.'}))
...
%     disp(strcat({'Startzeit '},num2str(uhr(4)),':',num2str(uhr(5)),':',num2str(uhr(6))))
    ...
    [raw_func_filenames_ra, dirs] = spm_select('List', direc_funct, '^a_.*\.nii$');
...
%     disp(strcat({'Startzeit '},num2str(uhr(4)),':',num2str(uhr(5)),':',num2str(uhr(6))))
    ...
    jobs{1}.spatial{1}.realign{1}.write.roptions.which = [2 1];%[0 1]; % Resliced Images ([0 1] > Only Mean Image; Default: [2 1] > All Images + Mean Image) 
    jobs{1}.spatial{1}.realign{1}.write.roptions.interp = 5; % Interpolation (Default: 4th Degree B-Spline) 
    jobs{1}.spatial{1}.realign{1}.write.roptions.wrap = [0 0 0]; % Wrapping (Default: No wrap) 
    jobs{1}.spatial{1}.realign{1}.write.roptions.mask = 1; % Masking (Default: Mask images) 
    jobs{1}.spatial{1}.realign{1}.write.roptions.prefix= 'r_';
    
    [raw_func_filenames_rars, dirs] = spm_select('List', direc_funct, '^a_.*\.nii$');
    filenames_for_rars = strcat(direc_funct,filesep,raw_func_filenames_rars); 
    
    
    jobs{1}.spatial{1}.realign{1}.write.data = cellstr(filenames_for_rars);
    ...
    disp(strcat({'@ Realignment-reslice for '},subjs(isub).name, {' done'}))
